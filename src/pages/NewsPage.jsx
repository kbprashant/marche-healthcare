...
const NewsPage = () => {
  ...
  // selected news card state — start empty
  const [selctedCardList, setSelctedCardList] = useState([]);
  const activeCard = selctedCardList?.[0] || null;

  // DB state for Social Media posts
  const [dbSocialPosts, setDbSocialPosts] = useState([]);
  ...
  useEffect(() => {
    ...
    else if (location.hash === "#newsandevents") setSelectedSection("newsandevents");
  }, [location]);
  useEffect(() => {
    if (location.hash) {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth <= 600) setSlideState({ noOfSlide: 1, navigation: false });
      else if (window.innerWidth <= 770) setSlideState({ noOfSlide: 2, navigation: false });
      else setSlideState({ noOfSlide: 3, navigation: true });
    };
    handleResize();
  ...
  // Fetch Social Media posts from DB
  useEffect(() => {
    let active = true;
    (async () => {
      try {
        ...
        <div style={{ width: "100vw" }}>
          <div className="social-links">
            <a href="https://www.linkedin.com/company/marche-healthcare/" target="_blank" rel="noopener noreferrer">
              <motion.svg whileHover={{ scale: 1.2 }} width="42" height="35" viewBox="0 0 24 24" fill="black">{linkedInSvg}</motion.svg>
            </a>
            <a href="https://x.com/info_march49738" target="_blank" rel="noopener noreferrer">
              <motion.svg whileHover={{ scale: 1.2 }} width="42" height="35" viewBox="0 0 24 24" fill="black">{twitterSvg}</motion.svg>
            </a>
            <a href="https://www.instagram.com/invites/contact/?igsh=jcoo1221g882&utm_content=uh81aej" target="_blank" rel="noopener noreferrer">
              <motion.svg whileHover={{ scale: 1.2 }} width="42" height="35" viewBox="0 0 24 24" fill="black">{instagramSvg}</motion.svg>
            </a>
            <a href="https://youtube.com/@marchehealthcare?si=Nktj1VOLNtZBrdmS" target="_blank" rel="noopener noreferrer">
              <motion.svg whileHover={{ scale: 1.2 }} width="42" height="42" viewBox="0 0 24 24" fill="black">{youtubeSvg}</motion.svg>
            </a>
          </div>
          ...
          <div className="newspagecardcontainer">
            ...
            >
              ...
              {!spLoading && spError && (
                <SwiperSlide key="error">
                  <div style={{ textAlign: "center", color: "crimson", padding: 40 }}>
                    {spError}. Showing sample card below.
                  </div>
                </SwiperSlide>
              )}
              ...
              const card = dbSocialPosts.length > 0
                ...
                return (
                  <SwiperSlide key={`sp-${card.id || idx}`}>
                    {card.link ? (
      ...
      <section className="headline">
        <div className="newsbanner">
          ...
          <p className="desc">
            ...
            Broadcast. Join us at upcoming events to collaborate and drive health equality&nbsp;forward
          </p>
        </div>
        ...
        <div id={SECTION_DATA[selectedSection].id} className="Media Center">

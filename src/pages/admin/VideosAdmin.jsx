...
const getToken = () => {
  try {
    return JSON.parse(localStorage.getItem("marche_admin_auth"))?.token || "";
  } catch {
    return "";
  }
};
    ...
    <>
      <div className="admin-page">
        <div className="admin-header">
          <SectionHeader
            title="Videos"
            content="Manage Product, Surgery and Training videos"
          />
          <div className="admin-actions">
            ...
            >
              Manage {TOP_CATEGORIES.find((t) => t.key === topCat)?.label}{" "}
              Sub-categories
            </button>
            <button
        ...
        <div className="tabbar">
          {TOP_CATEGORIES.map((c) => (
            <button
              key={c.key}
              className={`tab ${topCat === c.key ? "active" : ""}`}
              onClick={() => {
                setTopCat(c.key);
                setSubcat("all");
              }}
        ...
        {!!subcats.length && (
          <div className="subtabbar">
            <button
              className={`chip ${subcat === "all" ? "active" : ""}`}
            ...
            {subcats.map((sc) => (
              <button
                key={sc.id}
                className={`chip ${
                  subcat === String(sc.id) ? "active" : ""
                }`}
                onClick={() => setSubcat(String(sc.id))}
              >
                {sc.name}
              </button>
            ))}
              ...
              <div className="trow" key={row.id}>
                ...
                <div>
                  <img
                    ...
                    src={
                      row.thumbnail_url ||
                      (getYouTubeId(row.youtube_url)
                        ? ytThumb(getYouTubeId(row.youtube_url), "max")
                        : "/placeholder-thumb.png")
                    }
                    onError={(e) => {
                      // fallback to hqdefault if maxres not available
                      if (e.currentTarget.dataset.fbk !== "1") {
                        const id = getYouTubeId(row.youtube_url);
                        if (id) {
                          e.currentTarget.src = ytThumb(id, "hq");
                ...
                <div className="row-actions">
                  <button
                    className="btn ghost"
                    onClick={() =>
                      window.open(row.youtube_url || row.src_url || "#", "_blank")
                    }
                  >
                    View
                  </button>
